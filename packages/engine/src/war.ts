...
export interface Building {
  ...
  /** Units trained here this round, against the per-round limit. */
  trained: number;
}
...
function tilesBeside(state: MatchState, plot: Plot, n: number): Cell[] {
  const seeds: Cell[] = [];
  for (let r = plot.row - 1; r <= plot.row + plot.h; r++) {
...
export function payIncome(state: MatchState): MatchState {
  ...
  for (const b of Object.values(next.buildings)) b.trained = 0;
  return next;
}
...
/** Round 1: castle, barracks, two Pawns at the home mine, and the first income. */
export function newMatch(seed: number | string = 0): MatchState {
  ...
  for (const side of SIDES) {
    ...
    for (const slot of mineSlots(mine).slice(0, WAR.startPawns)) {
...
export function applyAction(state: MatchState, side: WarSide, action: Action): ActionResult {
  ...
  switch (action.type) {
    case "train": {
      ...
      if (b.trained >= WAR.trainPerRound[b.level]!) return fail(`the ${plot.kind} has trained all it can this round`);
      ...
      next.gold[side] -= cost;
      b.trained += 1;
      // A new Pawn goes straight to its home mine when there is a slot.
      const mine = homeMine(side);
      const toMine = cls === "pawn" && gatherers(next, mine.id, side).length < WAR.pawnsPerMine;
      addUnit(next, side, cls, at, toMine ? { type: "gather", mine: mine.id } : { type: "stop" });
