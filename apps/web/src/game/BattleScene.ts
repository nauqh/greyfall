// Battle playback. The engine already decided everything; this scene only
// replays its event log.
//
// Layout follows the autobattler convention rather than a side-on brawl: the
// board runs UP the screen, your army on the near half, the enemy's on the
// far half, their front rows meeting across the middle. The board is a plain
// square grid, 5 columns by 6 rows of TILE-sized cells, the Auto Chess shape.
// Units keep the pack's native pixel scale and the whole canvas is scaled up
// by Phaser's FIT, so the art never resamples.
//
// Playback does not begin the moment the scene opens: both armies stand on
// the board through a 3-2-1 count first, so you can read what you are up
...
/** Cell size. A 192px unit frame carries an ~80px body, so it fills a cell. */
const TILE = 84;
...
/**
 * The island, in whole 64px terrain tiles. The margins either side of the
 * board are what the scenery fills, so they are kept wide enough for a tree.
 *
 * The vertical placement is set by the far army's plate, not by taste: a unit
 * on row 0 carries its health pip ~100px above its cell centre, so the board
 * has to start low enough that the pip clears the plate. Sitting the board
 * straight under it, as it was, put every far-row pip behind the plate.
 */
const ISLAND: Rect = { x0: 128, y0: 168, x1: 832, y1: 744 };

const BOARD_W = COLS * TILE;
const BOARD_H = ROWS * TILE;
/** Centres of cell (0, 0); the board is centred on the canvas and the island. */
...
/**
 * How long a one-cell walk takes on screen. A unit acts once a second, so the
 * step is stretched to nearly fill that second: at 420ms it covered the
 * ground in a snap and then stood still for the remaining 580, which is what
 * made movement read as teleporting. At 880 the walk is almost continuous and
 * still lands before the unit's next turn.
 */
const STEP_MS = 880;
...
interface UnitView {
  ...
  alive: boolean;
  hp: number;
  /** What the pip is currently showing; it slides toward `hp`. */
  shownHp: number;
}
...
export class BattleScene extends Phaser.Scene {
  ...
  /** Playback is held until the 3-2-1 count finishes. */
  ...
  /**
   * Both armies are already standing on the board by the time this runs, so
   * the count is all that is needed to give the player a moment to read the
   * two lines before they close. Playback starts when it finishes.
   */
  private runIntro(): void {
    const steps = ["3", "2", "1", "FIGHT"];
    let i = 0;

    // A veil so the count reads over the armies without hiding them; it lifts
    // on the last beat, which is the cue that the board is live.
    const veil = this.add
  ...
  update(_time: number, delta: number): void {
    // Health pips and shadows ride along with their sprite while it tweens,
    // and the pip slides toward the new value instead of snapping to it.
    ...
    if (this.nextEvent >= this.queue.length) {
      this.finished = true;
      // A short settle so the last death lands before the result card does.
  ...
  /**
   * Cell centre in screen pixels. Snapshots and move events already carry
   * battle-grid coordinates, so no side mapping happens here: the engine put
   * side A on the near rows and side B on the far ones.
   */
  ...
  private drawGrid(): void {
    ...
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        ...
        // A touch of shade on the near half, so you can see which is yours.
    ...
    // The line the two front rows meet on.
  ...
  private spawnArmies(): void {
    // Far rows first, so nearer units naturally overlap them.
    ...
    for (const snap of order) {
      ...
      // Attacks are one-shots; hand back to idle when they finish.
  ...
  /**
   * A small health pip over each unit. The pack's own bar needs two 64px caps
   * before it draws anything, so it cannot go narrower than ~128px - far too
   * wide for an 84px cell. The army totals in the HUD use the pack bar; per-unit
   * pips stay hand-drawn, which is what TFT does over its units too.
   */
  ...
  private buildHud(): void {
    ...
    // Far side: a plate on the water above the island.
    ...
    // Near side: the player's plate, on a wood table below the island.
    ...
    // The speed toggle is the kit's small square button, which is a single
    // image rather than a nine-slice, so it keeps its shape at any size.
  ...
  /**
   * Pick the attack pose. The Lancer is the only class the pack gives vertical
   * thrusts, and now that the armies face up and down the board that is the
   * pose it mostly wants; a mainly sideways target still gets the level
   * thrust. Everyone else has one swing and is flipped to face the target.
   */
  private onAttack(ev: Extract<BattleEvent, { type: "attack" }>): void {
    ...
    if (attacker.snap.class === "archer") {
      // The shoot sheet ends at the release, so the arrow is a projectile the
      // scene flies itself - and it points where it is going.
  ...
  private onHit(ev: Extract<BattleEvent, { type: "hit" }>): void {
    ...
    // Hit feedback per the PRD: a white flash plus a small shake.
    target.sprite.setTintFill(0xffffff);
    this.time.delayedCall(80 / this.speed, () => {
      if (target.alive) target.sprite.clearTint();
    });
    // A single soft nudge. The old 45ms double-shake read as a jitter.
  ...
  private onHeal(ev: Extract<BattleEvent, { type: "heal" }>): void {
    ...
    // The pack ships a heal burst alongside the Monk's cast: play it on the
    // unit being mended, not on the Monk.
  ...
  private onDeath(ev: Extract<BattleEvent, { type: "death" }>): void {
    ...
    // Grey out, drift up, fade, and leave a dust puff: the soul goes, the
    // body returns to the ground. The PRD's death beat.
...
/**
 * Build the Phaser game for one battle. FIT scaling means the logical board
 * above is laid out once and then fitted to whatever the page gives it, so
 * the canvas is never cramped and the pixel art never resamples oddly.
 */
