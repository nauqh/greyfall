...
flowchart LR
  ...
  C --> D[Aftermath: report,<br/>healing]
...
3. **Spend:** train units, build, upgrade.
...
8. **Aftermath:** a round report (losses, gold mined, buildings damaged), and units at home heal.
...
- Units target the nearest enemy, ties to the lowest HP. Only an Attack order overrides it. There is no target priority setting.
...
- Each side has exactly 3 Pawns for the whole match; Pawns are never trained. A Pawn that dies stands beside the castle again at the start of the next round. Each uses 1 supply. A Pawn gathering at a mine earns 2 gold per round, paid at the start of the round. Income is per round rather than per trip so a short battle phase does not starve anyone; Pawns still walk and mine on the map during the battle.
...
- **Destroyed:** a building loses its upgrade and can be rebuilt on its plot. Units of its class stay, but no more can be trained until it stands again.
- **Upgrades** apply at once to every unit of that class, including those already in the field.

Each production building has one upgrade, to level 2. The castle has none.

| Building | Unlocks | Build cost | Upgrade to level 2 |
| --- | --- | --- | --- |
| Castle | Nothing; the main hall | Already built | None |
| Barracks | Warrior | Already built | 6 gold: +20% HP |
| Archery range | Archer | 4 gold | 6 gold: +1 range |
| Tower | Lancer | 4 gold | 6 gold: +20% HP |
| Monastery | Monk | 5 gold | 6 gold: +30% healing |
| House | +3 supply | 4 gold | None |
...
The AI's side is the Greying's host, one fixed roster from the pack's Enemy Pack. Each monster fills a knight class's role with the same cost, stats and counters, so the balance table stays one table and only the sprites differ. It upgrades its buildings with the same bonuses; an upgraded monster gets a tint rather than a new body.

| Role | Knight | Monster | Art notes |
| --- | --- | --- | --- |
| Melee | Warrior | Skull | Idle, run, attack |
| Tank | Lancer | Turtle | Idle, walk, attack; 320px frames like the Lancer |
| Ranged | Archer | Gnoll | Throws a bone, its own projectile |
| Support | Monk | Hex Shaman | No heal pose: heals with its attack pose and the Monk's heal effect |
| Worker | Pawn | Gnome | No mining pose: digs with its attack pose at the mine |

| Knight building | Monster building |
| --- | --- |
| Castle | Cave |
| Barracks | Goblin Hut |
...
| Tower | Pirate Tower |
| Monastery | Dead Tree |
| House | Gnome Hut, Fish Hut, and the Goblin Hut's other variants |

Monster buildings come in one colour only, and each monster has its own avatar for the selection panel.
...
- **The Greying closes in:** from round 12, the grey gnaws at both main halls at the end of every battle phase, for 5% of their max HP, growing by 5% each round (5%, 10%, 15%, ...). It hits both sides equally, so whoever has already damaged the other hall wins the race, and no match outlasts round 17. The HUD shows the rounds left before it starts, then each round's bite.
...
- **Buildings:** the pack has one image per building, so a building under construction is a pale copy of it and a destroyed one burns (fire and explosion effects) before it is removed.
...
- A battle phase returns an event log (`move`, `attack`, `hitBuilding`, `heal`, `fallBack`, `death`, `destroyed`) that the client plays back on the map.
...
| `matches` | `id`, `mode` (solo or duel), `seed`, `status`, `round`, `colours`, `state_json`, `deadline`, `missed_a`, `missed_b` |
...
| --- | --- | --- |
| `match.start` | Mutation | Start a solo match or open a duel, with a chosen colour |
...
| Risk | Mitigation |
| --- | --- |
| Scope creep in content | Freeze the roster at 5 classes and one monster host until launch |
